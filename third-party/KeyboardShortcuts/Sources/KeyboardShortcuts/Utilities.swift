...
extension String {
	...
	*/
	var localized: String {
		// LOCAL PATCH (vs. upstream): resolve the resource bundle via `.keyboardShortcutsResources`
		// instead of SwiftPM's generated `Bundle.module`. `Bundle.module`'s accessor only looks next
		// to the executable's `bundleURL` (invalid once wrapped in a signed `.app`) plus a hardcoded
		// build path, so it `fatalError`s in a distributed app the moment a `Recorder` renders. The
		// resolver below searches the app's real resource locations and degrades gracefully.
...
extension Bundle {
	/**
	The SwiftPM-generated `KeyboardShortcuts_KeyboardShortcuts.bundle`, resolved from wherever the host
	app actually ships it.

	`Bundle.module` can't be used here: `swift build` generates an accessor that only checks
	`Bundle.main.bundleURL/<name>.bundle` (the `.app` root — nothing signable lives there) and a
	hardcoded build-directory path (absent on any machine but the one that built it). Both miss, so it
	`fatalError`s. This searches the standard app resource locations instead and falls back to
	`.main` (in which case `NSLocalizedString` simply returns the key) rather than crashing.
	*/
	static let keyboardShortcutsResources: Bundle = {
		...
		let candidates = [
			...
			Bundle.main.bundleURL,                                      // App.app  (and CLI adjacency)
			Bundle(for: BundleFinder.self).resourceURL,                 // framework/embedded layouts
